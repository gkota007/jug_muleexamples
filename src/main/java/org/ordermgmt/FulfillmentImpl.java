...
public class FulfillmentImpl implements IFulfillment{
	...
	public ShippingOrderConfirmation putShippingOrder(@WebParam(name="customerId") String customerId,
													  @WebParam(name="shippingId") String shippingId,
			 										  @WebParam(name="order")  Order order){

		try {
			shippingOrder = new ShippingOrder(customerId, shippingId, order);
